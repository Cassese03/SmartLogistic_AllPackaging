    ...
    function cerca_articolo_inventario($barcode)
    {
        ...
        if (sizeof($articoli) > 0) {
            ...
            if (sizeof($disponibilita) > 0) {
                ...
                $prova = DB::SELECT('SELECT ISNULL(sum(QuantitaSign),0) as disponibilita,Cd_ARLotto,Cd_MG from MGMOV where Cd_MGEsercizio = ' . date('Y') - 1 . ' and Cd_AR = \'' . $articolo->Cd_AR . '\' and Cd_ARLotto IS NOT NULL group by Cd_ARLotto, Cd_MG HAVING SUM(QuantitaSign)!= 0  ');
            }
